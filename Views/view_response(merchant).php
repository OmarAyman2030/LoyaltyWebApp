...
if(isset($_GET["logout"])){
  require_once("../Controlles/Authcontroller.php");
    ...
    $products=$productC->get("response");
...
<?php require_once("../Models/merchant_nav.php");?>
  ...
  <!--end of nav-->
      ...
      <div class="container mt-4">
        <h2 style="color: black;">Resposne Table</h2>
          ...
          <tbody>
            <<?php
              foreach($products as $p){
                  if ($p["Merchant_N"] == $_SESSION["username"]){
              ?>
                      <tr>
                          <td><?php echo $p["product_N"];?></td>
              <?php
                      if ($p["status"] == "Accepted"){?>
                          <td class="text-success"><?php echo $p["status"];?></td>
              <?php
                      } else {?>
                          <td class="text-danger"><?php echo $p["status"];?></td>
              <?php
                      }?>
                      </tr>
              <?php
